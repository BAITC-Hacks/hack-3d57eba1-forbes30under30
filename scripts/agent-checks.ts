...
function firstTools(index: number): Response | undefined {
  return index === 0 ? tool("score_set") : index === 1 ? tool("get_contributions") : undefined;
}
  ...
  try {
    ...
    await check("API сохраняет расчёт без ключа и отклоняет неверный набор", async () => {
      ...
      try {
        ...
        const body = await success.json() as { calc: { score: number }; report: unknown; steps: unknown[]; aiError?: string };
    ...
    await check("ошибка схемы исправляется единственным повтором", async () => {
      respond = (_request, index) => firstTools(index) ?? (index === 2 ? final({ ...validReport, strengths: "не массив" }) : final());
      const result = await runAgent(example);
      assert.ok(result.report, result.aiError);
    ...
    await check("expectedDelta без расчёта альтернативы отклоняется", async () => {
      respond = (_request, index) => firstTools(index) ?? final({
        ...validReport,
    ...
    await check("рекомендация без прогноза: null преобразуется в необязательное поле", async () => {
      respond = (_request, index) => firstTools(index) ?? final({
        ...validReport,
        recommendations: [{ change: "Рассмотреть меры транспорта", expectedDelta: null, why: "Это направление не затронуто." }],
      });
      const result = await runAgent(example);
      assert.ok(result.report, result.aiError);
      assert.equal("expectedDelta" in result.report.recommendations[0], false);
    });

    await check("ожидаемая дельта разрешена после расчёта альтернативы", async () => {
      const alternative = scenarios[2].decisions;
      const expectedDelta = Number((score(alternative).score - score(example).score).toFixed(2));
      respond = (_request, index) => firstTools(index) ?? (index === 2
      ...
      const result = await runAgent(example);
      assert.ok(result.report, result.aiError);
      assert.equal(result.report.recommendations[0].expectedDelta, expectedDelta);
      assert.equal(requests.length, 4);
    });
    ...
    await check("после восьми инструментов агент запрашивает только финал", async () => {
      ...
      assert.equal(result.steps.filter((step) => ["score_set", "get_contributions", "validate_set", "get_district_profile"].includes(step.name)).length, 8);
    });
    ...
    await check("девятый инструмент запрещён даже при нарушении моделью", async () => {
      ...
      assert.ok(result.steps.filter((step) => ["score_set", "get_contributions", "validate_set", "get_district_profile"].includes(step.name)).length <= 8);
    });
    ...
    await check("неизвестный район не мешает получить отчёт по исходному расчёту", async () => {
      ...
      const result = await runAgent(example);
      assert.ok(result.report, result.aiError);
      const lastToolResult = requests[3].messages.filter((message) => message.role === "tool").at(-1);
